...
async function me(req, res) {
  ...
  if (role === 'company') {
    ...
    return res.json({ role: 'company', user: data });
  }

  if (role === 'driver') {
    const { data, error } = await supabase
      .from('driver')
      .select('driver_id, company_id, driver_name, driver_code, phone, profile_image, current_score, created_at')
      .eq('driver_id', id)
      .single();
    ...
    return res.json({ role: 'driver', user: data });
  }
